...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \textbf{Решение:} \\
    Данное регулярное выражение эквивалентно \[ (a \mid b)^+ (a \mid b)^+\], т.к. оба автомата задают язык <<произвольная последовательность a и b длины не менее $2$>>. \\
    \begin{myauto}
      \node[state]           (q_1)                {$q_1$};
    ...
    \begin{myauto}
      ...
      ;
    \end{myauto}
     Автомат минимален, т.к. во всех словах есть хотя бы две буквы, а значит меньше 3 состояний быть не может. \\
  \end{enumerate}
  ...
  \end{myauto}
  \textbf{Решение:} \\
    Сначала может быть произвольное число букв: $\text{(a | b | c)}^*$ \\
    Потом должна быть последовательность $\text{x (y | z)}^*\text{x}$, где $x, y, z$ соответствуют какой-то перестановке $a, b, c$. \\
    Получается: \[ (a \mid b \mid c)^* ((a (b\mid c)^* a) \ \mid \ (b (a\mid c)^* b) \  \mid \ (c (a\mid b)^* c)) \]
  ...
  \textbf{Решение:}\\
  Обнаружим противоречие с леммой-насосом. Хотим для $\forall n$ предъявить $\alpha \in L \colon |\alpha| \ge n$ и $\forall u, w, v \colon a = uvw \land |uv| \le n \land |v| \ge 1 \; \; \exists i \colon uv^iw \not \in L$. \\ \\
  Для произвольного $n$ рассмотрим строчку $1^n 00 1^n \in L$. Рассмотрим произвольное разбиение $\alpha = uvw$. По условию леммы, $|uv| \le n$, поэтому $v$ является подстрокой первых $n$ единичек и не содержит нулей. Тогда, если рассмотреть строчку $uv^kw$, то она будет иметь вид $1^m 00 1^n \mid m > n$. Но такая строчка не лежит в нашем языке, значит, он не регулярный.
  ...
  \textbf{Решение:} \\
  Обнаружим противоречие с леммой-насосом. Хотим для $\forall n$ предъявить $\alpha \in L \colon |\alpha| \ge n$ и $\forall u, w, v \colon a = uvw \land |uv| \le n \land |v| \ge 1 \; \; \exists i \colon uv^iw \not \in L$. \\ \\
   Для произвольного $n$ рассмотрим строчку $b^n aa a^n$. Рассмотрим произвольное разбиение $\alpha = uvw$. По условию леммы, $|uv| \le n$, поэтому $v$ является подстрокой первых $n$ <<бэшек>>, причем непустой. Рассмотрим строчку $uv^0w$, она будет иметь вид $b^m aa a^n \mid m < n$, то есть, не будет принадлежать языку. Значит, он не регулярный.
\end{enumerate}
